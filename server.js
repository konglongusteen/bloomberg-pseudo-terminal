...
// Yahoo Finance chart data (candles)
...
app.get('/api/yahoo/quote', async (req, res) => {
    ...
    try {
        ...
        if (data && data.chart && data.chart.result && data.chart.result[0]) {
            ...
            res.json({ success: true, price, change, changePct });
        } else {
            res.status(404).json({ error: 'No data from Yahoo' });
        }
    } catch (err) {
    ...
    try {
        ...
        parseString(xml, (err, result) => {
            ...
            const news = items.slice(0, 15).map(item => ({
                ...
                source: item.source ? (Array.isArray(item.source) ? item.source[0] : item.source) : 'Yahoo Finance'
            }));
            res.json({ news });
        });
    } catch (err) {
...
app.post('/api/copilot/query', aiRateLimiter, async (req, res) => {
    ...
    try {
        const completion = await groqClient.chat.completions.create({
            ...
            messages: [
                { role: 'system', content: 'You are a financial AI assistant. Answer questions about markets, stocks, and trading. Be concise and factual.' },
